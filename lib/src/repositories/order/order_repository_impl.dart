...
class OrderRepositoryImpl implements OrderRepository {
  ...
  Future<void> changeStatus(int id, OrderStatus status) async {
    try {
        await _dio.auth().put('/orders/$id', 
        data: {
            'status': status.acronym,
          },
        );
      } on DioError catch (e, s) {
      log('Erro ao alterar status do pedido para ${status.acronym}', error: e, stackTrace: s);
  ...
  Future<List<OrderModel>> findAllOrders(DateTime date, OrderStatus? status) async {
    try {
        final orderResponse = await _dio.auth().get('/orders', 
        queryParameters: {
            'date': date.toIso8601String(),
            if(status != null) 'status': status.acronym
          },
        );
        return orderResponse.data.map<OrderModel>((p) => OrderModel.fromMap(p)).toList();
      } on DioError catch (e, s) {
